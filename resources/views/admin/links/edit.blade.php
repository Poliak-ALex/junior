        ...
        <div class="col-md-12">
            <div class="card">
                <h2 class="card-header bg-secondary text-white">
                    Edit URL
                    <a class="btn btn-primary" style="position: absolute; top: 20%; right: 1%;" href="{{ url('admin/user/'. $userId . '/links') }}"> Back</a>
                </h2>
                <div class="card-body">    
                    <form action="{{ route('links.update',[$userId,$link->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                         <div class="row">
                            {{-- Url field --}}
